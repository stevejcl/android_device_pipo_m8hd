...
#check connection
./rkflashtool/rkflashtool p &> /dev/null || $(echo "rkflashtool can not connect. please make sure device is in bootloader mode!"; exit 0)
...
MD5SUM_CALC=`./rkflashtool/rkflashtool p 2>/dev/null|grep CMDLINE|md5sum`
#echo "$MD5SUM_CALC" "$MD5SUM_OK" 
